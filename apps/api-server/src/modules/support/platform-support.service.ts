...
import { BotPlatform, OrderSourcePlatform, Prisma, UserRole, UserStatus } from "@prisma/client";
...
const SUPPORT_RULES: SupportRule[] = [
  {
    topic: "充值未到账",
    keywords: ["充值", "到账", "余额", "转账", "加钱", "未到账", "审核", "截图"],
    answer:
      "充值目前是人工审核。请在充值页提交金额、转账截图和备注，管理员审核通过后余额会进入账户。金额填错或截图不清楚时，请把账号邮箱和截图发给人工客服处理。"
  },
  {
    topic: "找陪玩",
    keywords: ["找陪玩", "下单", "派单", "预约", "试音", "上分", "带我", "陪打", "陪练"],
    answer:
      "可以，我先帮你记录需求。请补充：游戏、模式、预计时长、预算、是否要试音、是否偏好男声/女声。确认后客服会安排候选陪玩给你选择。",
  ...
  },
  {
    ...
    answer:
      "如果你想先试音选人，客服会把需求整理成试音派单，让有空的陪玩报名，再把候选人发给你挑选。确认后才会转成正式订单。",
    handoffRequired: true
  },
  {
    topic: "退款投诉",
    keywords: ["退款", "投诉", "不满意", "纠纷", "取消", "退钱"],
    answer:
  ...
  {
    topic: "陪玩入驻",
    keywords: ["陪玩注册", "入驻", "成为陪玩", "考核", "陪玩端"],
    answer:
      "陪玩不能自行注册。请联系 VX/KOOK/Discord 客服进行资料审核和试音考核，通过后由管理员创建陪玩账号、设置资料并上架。"
  },
  {
    topic: "提现",
    keywords: ["提现", "支付宝", "收款", "打款", "收益"],
    answer:
      "陪玩收益会在订单完成结算后进入可提现收入。陪玩端可设置支付宝收款姓名和账号，提交提现后由管理员人工审核并打款。"
  },
  {
    topic: "账号绑定",
    keywords: ["绑定", "kook", "discord", "dc", "频道", "客服", "登录"],
    answer:
      "客户可以用邮箱、KOOK 或 Discord 登录/绑定。KOOK/Discord 主要用于客服沟通、试音频道、人工派单和订单通知；绑定后后台可以追踪订单来源。"
  },
  {
    topic: "优惠推荐",
    keywords: ["优惠", "推荐", "邀请码", "老带新", "返利", "奖励", "首单", "首充"],
    answer:
      "平台支持首充赠送、客户老带新和陪玩带客奖励。注册或下单时填写邀请码，符合条件后系统会按后台配置发放奖励，具体金额以活动页和客服确认为准。"
  }
];
...
export class PlatformSupportService {
  ...
  private async maybeCreateDispatchDraft(input: PlatformSupportMessage, customerId: string | undefined, message: string) {
    if (process.env.AI_AUTO_DISPATCH_ENABLED !== "true") {
      return {
        draft: null,
        reply:
          "我已记录你的陪玩需求。请继续补充游戏、模式、时长、预算和语音偏好；人工客服确认后会安排试音/派单。"
      };
    }
    ...
    if (!serviceAdmin) {
      return {
        draft: null,
        reply: "我已记录你的需求，但后台还没有可用管理员账号。请人工客服先处理这条派单。"
      };
    }
    ...
    return {
      ...
      reply: `已为你生成试音派单 ${result.draft.draftNo}，我已经通知陪玩频道报名。稍后会把候选陪玩发给你选择；充值、退款和余额问题仍由人工客服确认。`
    };
  }
  ...
  private async tryOpenAiSupportAnswer(message: string) {
    ...
    try {
          ...
          input: [
            {
              role: "developer",
              content:
                "你是 May猫饼电竞陪玩俱乐部客服。只回答平台流程、下单、试音、充值说明、绑定、入驻、提现规则。涉及余额修改、退款、提现、封号、投诉结论时必须提示转人工，不允许承诺已经处理。中文简洁回复。"
            },
            {
              role: "user",
              content: message
      ...
      return data.output_text?.trim() || data.output?.flatMap((item) => item.content ?? []).map((item) => item.text).filter(Boolean).join("\n").trim() || null;
    } catch {
      return null;
    }
  }
  ...
  private isDispatchIntent(message: string) {
    return /(找陪玩|下单|派单|预约|试音|上分|带我|陪打|陪练|来个|安排)/i.test(message);
  }
